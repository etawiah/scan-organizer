...
class ScanProcessor:
    ...
    def __init__(self, scan_folder, use_ai=False):
        ...
        # Check for API key in environment variable if AI is enabled
        if self.use_ai:
            self.api_key = os.environ.get('ANTHROPIC_API_KEY')
            if not self.api_key:
                raise ValueError("AI classification requested but ANTHROPIC_API_KEY environment variable not found")
    
    def validate_system_requirements(self):
        """Validate that all required tools are available before processing"""
        print("\n" + "="*60)
        ...
        # Test Tesseract
        print("\n[1/3] Testing Tesseract OCR...")
        try:
            test_result = pytesseract.get_tesseract_version()
            print(f"   ✓ Tesseract found: v{test_result}")
        except Exception as e:
            error_msg = f"Tesseract not found or not working: {e}"
            print(f"   ✗ {error_msg}")
            errors.append(error_msg)
        
        # Test Poppler (for PDFs)
        print("\n[2/3] Testing Poppler (PDF support)...")
        try:
            # Try to import and use a simple test
            from pdf2image.exceptions import PDFInfoNotInstalledError
            # Create a minimal test - this will fail if poppler isn't available
            import subprocess
            ...
            if result.returncode == 0 or 'poppler' in result.stderr.lower():
                print(f"   ✓ Poppler found")
            else:
                raise Exception("pdftoppm command not working")
        except FileNotFoundError:
            error_msg = "Poppler not found in PATH. PDF processing will fail."
            print(f"   ✗ {error_msg}")
            errors.append(error_msg)
        except Exception as e:
            error_msg = f"Poppler not working: {e}"
            print(f"   ✗ {error_msg}")
            errors.append(error_msg)
        
        # Test AI API if enabled
        if self.use_ai:
            print("\n[3/3] Testing Anthropic API connection...")
            try:
                ...
                error_msg = f"API authentication failed: {e.code} - {error_body}"
                print(f"   ✗ {error_msg}")
                errors.append(error_msg)
            except Exception as e:
            ...
            print("\n[3/3] AI classification disabled - skipping API test")
        ...
        if errors:
            ...
            print("="*60)
            print("\nThe following errors must be fixed before processing:\n")
            for i, error in enumerate(errors, 1):
                print(f"{i}. {error}")
            print("\n" + "="*60)
            raise ValueError("System validation failed. Please fix the errors above before continuing.")
        else:
...
DO NOT include any text outside the JSON. DO NOT use markdown code blocks."""
    ...
    def process_file(self, file_path):
        ...
        # If no text extracted, this is a critical error - stop processing
        if not text:
            raise RuntimeError(f"CRITICAL: Failed to extract text from {file_path}. OCR is not working properly. Stopping to prevent data loss.")
        
        print(f"Extracted {len(text)} characters of text")
        
        # Get classification (AI or fallback)
        if self.use_ai:
            print("Classifying with AI...")
        else:
            print("Classifying with keyword matching...")
        
        classification = self.get_ai_classification(text, file_path.name)
        
        # Check if classification failed when AI was requested
        if self.use_ai and classification.get('confidence') == 'low':
            # This means AI failed and fallback was used - this shouldn't happen after validation
            raise RuntimeError(f"CRITICAL: AI classification failed for {file_path}. Stopping to prevent incorrect categorization.")
        ...
        try:
            ...
            raise RuntimeError(f"CRITICAL: Failed to move file {file_path}: {e}")
...
def main():
    ...
    print("1. Keyword-based (Free, works offline)")
    print("2. AI-powered (Requires Anthropic API key, more accurate)")
    
    use_ai_choice = input("\nChoose classification method (1/2) [default: 1]: ").strip()
    use_ai = (use_ai_choice == '2')
    
    if use_ai and not os.environ.get('ANTHROPIC_API_KEY'):
        print("\n" + "="*60)
        print("AI CLASSIFICATION SETUP")
        print("="*60)
        print("\nTo use AI classification, you need to set the ANTHROPIC_API_KEY")
        print("environment variable. You can:")
        print("\nWindows:")
        print("  setx ANTHROPIC_API_KEY \"your-api-key-here\"")
        print("\nLinux/Mac:")
        print("  export ANTHROPIC_API_KEY=\"your-api-key-here\"")
        print("\nGet your API key from: https://console.anthropic.com/")
        print("\nFalling back to keyword-based classification for now...")
        use_ai = False
        input("\nPress Enter to continue...")
    
    # Initialize processor
    try:
        processor = ScanProcessor(scan_folder, use_ai=use_ai)
    except ValueError as e:
        print(f"Error: {e}")
        sys.exit(1)
    
    # CRITICAL: Validate system requirements before processing any files
    try:
        processor.validate_system_requirements()
    except ValueError as e:
        print(f"\n{e}")
        sys.exit(1)
